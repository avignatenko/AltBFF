...
struct cb_stuff
{
    cb_stuff(list<vector<string>>& _cache, CSVread::Flags& _flags, bool& _error_pending, std::string& _error_msg,
             bool& _end_record_not_terminated, uintmax_t& pending, uintmax_t& requested)
        : _cache(_cache),
          _flags(_flags),
          _error_pending(_error_pending),
...
static void Callback_Record(int terminator, void* userptr)
{
    ...
    if (s->_error_pending || ((terminator == CSV_CR) && (s->_flags & CSVread::process_empty_records)))
    {
        return;
    }
...
CSVread::CSVread()
    : buffer_size(_buffer_size),
      ...
      has_utf8_bom(_has_utf8_bom),
      record_num(_record_num),
      end_record_num(_end_record_num),
      end_record_not_terminated(_end_record_not_terminated),
      fields(_fields)
{
    if (!Init()) return;
}
...
{
    if (_buffer && (_buffer_size == bytes)) return true;
...
bool CSVread::Associate(istream* stream, const Flags flags /* = none */)
{
    ...
    */
    ...
    if (len > 0)
    {
        if (!(_flags & skip_utf8_bom_check) && (len >= 3) && (p[0] == '\xEF') && (p[1] == '\xBB') && (p[2] == '\xBF'))
        {
            _has_utf8_bom = true;
        }
        ...
        if (!_has_utf8_bom || (len > 3))
        {
            ...
            if (csv_parse(parse_obj, adjusted_p, adjusted_len, Callback_Field, Callback_Record, &args) != adjusted_len)
            {
                if (!_error_pending)
                {
                    _error_pending = true;
