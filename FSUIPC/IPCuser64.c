...
#define MAX_SIZE \
    0x7F00  // Largest data (kept below 32k to avoid
...
BOOL FSUIPC_Open(DWORD dwFSReq, DWORD* pdwResult)
{
    ...
    m_hMap = CreateFileMapping(INVALID_HANDLE_VALUE,  // use system paging file
                               NULL,                  // security
...
BOOL FSUIPC_Process(DWORD* pdwResult)
{
    ...
    while ((++i < 10) && !SendMessageTimeout(m_hWnd,      // FS6 window handle
    ...
    while (*pdw)
    {
        ...
        case F64IPC_READSTATEDATA_ID:
            ...
            if (pHdrR->pDest && pHdrR->nBytes) CopyMemory(pHdrR->pDest, m_pNext, pHdrR->nBytes);
...
BOOL FSUIPC_ReadCommon(BOOL fSpecial, DWORD dwOffset, DWORD dwSize, void* pDest, DWORD* pResult)
{
    ...
    {
        if (fSpecial)
            CopyMemory(&m_pNext[sizeof(F64IPC_READSTATEDATA_HDR)], pDest, dwSize);
        else
            ZeroMemory(&m_pNext[sizeof(F64IPC_READSTATEDATA_HDR)], dwSize);
    }
